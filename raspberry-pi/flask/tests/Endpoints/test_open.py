...
class TestOpenEndpoint(TestCase):
    ...
    def test_success(self):
        with mock.patch.multiple(Motor, status=Mock(return_value=True), open=Mock(return_value=None)):
            request_headers = {"Authorization": "Bearer testing-token"}
            response = self.app.put("/api/v1/blind/open", headers=request_headers)
    ...
    def test_correct_json_format_close(self):
        with mock.patch.multiple(Motor, status=Mock(return_value=False), open=Mock(return_value=None)):
            request_headers = {"Authorization": "Bearer testing-token"}
            response = self.app.put("/api/v1/blind/open", headers=request_headers)
            response_json = response.json
